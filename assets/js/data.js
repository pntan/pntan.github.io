const portfolioData = {

    profile: {
        name: "Phan Nhật Tân",
        role: "Creative Web Developer",
        avatar: "./assets/media/avt.png",

        description:
            "Sinh viên CNTT trường Cao đẳng Lý Tự Trọng. Đam mê phát triển Web, Automation, AI và các sản phẩm mang lại giá trị thực tế."
    },

    socials: [
        {
            name: "Facebook",
            url: "https://facebook.com/pntan215"
        },
        {
            name: "CodePen",
            url: "https://codepen.io/TanPhan"
        },
        {
            name: "Email",
            url: "mailto:nhattanphan2014@gmail.com"
        }
    ],

    skills: [
        { name: "HTML/CSS", level: 95 },
        { name: "JavaScript", level: 90 },
        { name: "PHP/MySQL", level: 85 },
        { name: "WordPress", level: 85 },
        { name: "Python", level: 80 },
        { name: "C# Winform", level: 75 }
    ],

    achievements: [
        {
            year: "2022",
            title: "Giải 3 HSG Nghề Cấp TP"
        },
        {
            year: "2023",
            title: "Học bổng Sinh Viên Giỏi"
        },
        {
            year: "2024",
            title: "Giải 4 Thuật Toán Khoa"
        }
    ],

    projects: [
        {
            title: "Bot Discord Đa Năng",
            description:
                "Bot Discord hỗ trợ AI Chat, Music, Utility và quản trị server.",
            stack: ["Python", "Discord API", "AI"]
        },
        {
            title: "Quản Lý Quán Cafe",
            description:
                "Hệ thống quản lý bán hàng, kho và doanh thu.",
            stack: ["PHP", "MySQL", "SASS"]
        },
        {
            title: "Quản Lý Thư Viện",
            description:
                "Quản lý sách, độc giả và phiếu mượn trả.",
            stack: ["C#", "SQL Server"]
        }
    ]
};